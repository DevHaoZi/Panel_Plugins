...
namespace Plugins\Php74\Controllers;
...
class Php74Controller extends Controller
{

    public function status(): JsonResponse
    {
        $command = 'systemctl status php-fpm-74';
    ...
    {
        $command = 'systemctl restart php-fpm-74';
        shell_exec($command);
    ...
    {
        $command = 'systemctl reload php-fpm-74';
        shell_exec($command);
    ...
    public function getConfig(): JsonResponse
    {
        $res['code'] = 0;
        $res['msg'] = 'success';
        $res['data'] = @file_get_contents('/www/server/php/74/etc/php.ini');
        return response()->json($res);
    }
    ...
    public function saveConfig(Request $request): JsonResponse
    {
        ...
        // 写入配置
        file_put_contents('/www/server/php/74/etc/php.ini', $config);
        // 重载PHP-7.4
        shell_exec('systemctl reload php-fpm-74');
        $res['data'] = 'PHP-7.4主配置已保存';
        return response()->json($res);
    }

    public function load()
    {
        $raw_status = HTTP::get('http://127.0.0.1/phpfpm_74_status')->body();
        $res['data'][0]['name'] = '应用池';
        // 正则匹配pool
    ...
    public function errorLog()
    {
        $res['code'] = 0;
        $res['msg'] = 'success';
        $res['data'] = shell_exec('tail -n 100 /www/server/php/74/var/log/php-fpm.log');
        //如果data为换行符，则令返回空
        if ($res['data'] == "\n") {
            $res['data'] = '';
        }
    ...
    public function cleanErrorLog()
    {
        $res['code'] = 0;
        $res['msg'] = 'success';
        shell_exec('echo "" > /www/server/php/74/var/log/php-fpm.log');
        return response()->json($res);
    }
    ...
    public function slowLog(): JsonResponse
    {
        $res['code'] = 0;
        $res['msg'] = 'success';
        $res['data'] = shell_exec('tail -n 100 /www/server/php/74/var/log/slow.log');
        //如果data为换行符，则令返回空
        if ($res['data'] == "\n") {
            $res['data'] = '';
        }
    ...
    public function cleanSlowLog(): JsonResponse
    {
        $res['code'] = 0;
        $res['msg'] = 'success';
        shell_exec('echo "" > /www/server/php/74/var/log/slow.log');
        return response()->json($res);
    }
    ...
    public function getExtensionList(): JsonResponse
    {
        ...
        // 获取本地拓展列表
        $rawExtensionList = shell_exec('php-74 -m');
    ...
    public function installExtension(Request $request): JsonResponse
    {
        ...
        // 入库等待安装
        $task = new Task();
        $task->name = '安装PHP-74拓展 '.$extensionData['name'];
    ...
    public function uninstallExtension(Request $request): JsonResponse
    {
        ...
        // 入库等待安装
        $task = new Task();
        $task->name = '卸载PHP-74拓展 '.$extensionData['name'];
    ...
    private function getRemoteExtension($cache = true)
    {
        // 判断刷新缓存
        if (!$cache) {
            Cache::forget('php74ExtensionList');
        }
        if (!Cache::has('php74ExtensionList')) {
            return Cache::remember('php74ExtensionList', 3600, function () {
                $response = Http::get('https://api.panel.haozi.xyz/api/phpExtension/list', ['version' => '74']);
                // 判断请求是否成功，如果不成功则抛出异常
                if ($response->failed()) {
            ...
            // 从缓存中获取access_token
            return Cache::get('php74ExtensionList');
        }
    }
}
